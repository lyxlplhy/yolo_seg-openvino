...
void YOLOV8::detect(cv::Mat& frame) {
    ...
    std::cout << "后处理时间: " << duration << " 毫秒" << std::endl;
...
void YOLOV8::initialmodel() {
    ...
    //    ppp.output(i).tensor().set_element_type(ov::element::f32);  // 直接链式调用，避免拷贝
    //}
    //model = ppp.build();
